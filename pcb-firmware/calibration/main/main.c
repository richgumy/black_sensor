...
***UNSTABLE VERSION COMPILING WITH THIS PROJECT***

ADC read program!

The PCB firmware is all written in C for the ESP32-WROOM32E SoC. This is an adpatation of an ERT data collection program used soley to gather differential input ADC data through two MUX'd electrodes.
...
static const uint8_t ert_mode = CALIBRATE_ADC; // <- SET ELECTRODE DRIVE PATTERN MODE HERE //
...
void init_ERT_mode (void) {
    #if (ert_mode == STANDBY)
        printf("MODE = STANDBY\n");
    #elif (ert_mode == CALIBRATE_ADC)
        isrc_elec = 14; // current source electrodes not used in this scenario, so set them != vp_elec and vn_elec
        isnk_elec = 15;
        vp_elec = 0;
        vn_elec = 1;
        printf("MODE = CALIBRATE_ADC\n");
