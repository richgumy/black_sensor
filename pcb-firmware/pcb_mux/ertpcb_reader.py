...
def log_eit_data(ser_handle, tot_cycles=15, v_meas_max_V=20, i_src_A=1e-3, num_elecs=16):
    ...
    while eit_cycles < tot_cycles:
        ...
        while v_meas != b'A' and not bad_frame_flag:
            ...
            i_meas += 1
        print(len(v_buf_frame_bit),bad_frame_flag)
        if len(v_buf_frame_bit) == 257:
            print(v_buf_frame_bit)
            time.sleep(0.5)
        if (not bad_frame_flag) and (len(v_buf_frame_bit) == num_elecs**2):
            for v in range(len(v_buf_frame_bit)):
                v_buf_V.append((Vadc2Vreal(float(v_buf_frame_bit[v]))))
            ...
            _ = 0
            # print(f"bad_frame_flag={bad_frame_flag} len(v_buf_frame_bit)={len(v_buf_frame_bit)}")


def main(tv_buf_s, v_buf_V, i_buf_A, tot_cycles, i_src_A, nplc, v_max_V, num_elecs=16):
...
if __name__ == "__main__":
    ...
    try:
        ...
        main(tv_buf_s, v_buf_V, i_buf_A, tot_cycles, i_src_A, nplc, v_max_V)
